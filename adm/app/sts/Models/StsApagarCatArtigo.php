...
class StsApagarCatArtigo {
    ...
    public function apagarCatArtigo($DadosId = null) {
        ...
        if ($this->Resultado) {
            ...
            if ($apagarCatArtigo->getResultado()) {
                $_SESSION['adms_msg'] = "<div class='alert alert-success'>Categoria de artigo apagado com sucesso!</div>";
                $this->Resultado = true;
            } else {
                $_SESSION['adms_msg'] = "<div class='alert alert-danger'>Erro: Categoria de artigo não foi apagado!</div>";
    ...
    private function verfArtigoCad() {
        ...
        if ($verArtigo->getResultado()) {
            $_SESSION['adms_msg'] = "<div class='alert alert-danger'>Erro: Categoria de artigo não pode ser apagada, há artigo cadastrado com esse categoria de artigo!</div>";
            $this->Resultado = false;
        } else {
            $this->Resultado = true;
        }
