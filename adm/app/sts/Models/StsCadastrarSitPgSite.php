...
class StsCadastrarSitPgSite
{
    ...
    private function inserirSitPgSite()
    {
        ...
        if ($cadSitPgSite->getResultado()) {
            $_SESSION['adms_msg'] = "<div class='alert alert-success'>Situação de página do site cadastrado com sucesso!</div>";
            $this->Resultado = true;
        } else {
            $_SESSION['adms_msg'] = "<div class='alert alert-danger'>Erro: Situação de página do site não foi cadastrado!</div>";
            $this->Resultado = false;
        }
    }
