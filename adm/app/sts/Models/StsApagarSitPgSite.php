...
class StsApagarSitPgSite {
    ...
    public function apagarSitPgSite($DadosId = null) {
        ...
        if ($this->Resultado) {
            ...
            if ($apagarSitPgSite->getResultado()) {
                $_SESSION['adms_msg'] = "<div class='alert alert-success'>Situação de página do site apagado com sucesso!</div>";
                $this->Resultado = true;
            } else {
                $_SESSION['adms_msg'] = "<div class='alert alert-danger'>Erro: Situação de página do site não foi apagado!</div>";
    ...
    private function verfPgCad() {
        ...
        if ($verPg->getResultado()) {
            $_SESSION['adms_msg'] = "<div class='alert alert-danger'>Erro: Situação de página do site não pode ser apagada, há páginas do site cadastrada com esse situação!</div>";
            $this->Resultado = false;
        } else {
            $this->Resultado = true;
        }
