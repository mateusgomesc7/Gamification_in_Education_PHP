...
class AdmsEditarPerfil
{
    ...
    private function valCampos()
    {
        ...
        $EditarUnico = true;
        $valEmailUnico->valEmailUnico($this->Dados['email'], $EditarUnico, $_SESSION['adms_usuario_id']);

        $valUsuario = new \App\adms\Models\helper\AdmsValUsuario();
        $valUsuario->valUsuario($this->Dados['usuario'], $EditarUnico, $_SESSION['adms_usuario_id']);
    ...
    private function valFoto()
    {
        if (empty($this->Foto['name'])) {
            ...
            $uploadImg->uploadImagem($this->Foto, 'assets/imagens/usuario/' . $_SESSION['adms_usuario_id'] . '/', $this->Dados['imagem'], 150, 150);
            if ($uploadImg->getResultado()) {
                $apagarImg = new \App\adms\Models\helper\AdmsApagarImg();
                $apagarImg->apagarImg('assets/imagens/usuario/' . $_SESSION['adms_usuario_id'] . '/' . $this->ImgAntiga);
                $this->updateEditPerfil();
            } else {
                $this->Resultado = false;
            }
    ...
    private function updateEditPerfil()
    {
        ...
        if ($upAltSenha->getResultado()) {
            $_SESSION['adms_usuario_nome'] = $this->Dados['nome'];
            $_SESSION['adms_usuario_email'] = $this->Dados['email'];
            $_SESSION['adms_usuario_imagem'] = $this->Dados['imagem'];
            $_SESSION['adms_msg'] = "<div class='alert alert-success'>Perfil atualizado com sucesso!</div>";
            $this->Resultado = true;
        } else {
            $_SESSION['adms_msg'] = "<div class='alert alert-danger'>Erro: O perfil não foi atualizado!</div>";
            $this->Resultado = false;
        }
    }
