...
class AdmsLibMenu
{
    ...
    public function libMenu($DadosId = null)
    {
        ...
        if ($this->DadosNivAcPg) {
            ...
            $_SESSION['adms_msg'] = "<div class='alert alert-danger'>Erro: Não foi alterado a situação do menu!</div>";
    ...
    private function altPermi()
    {
        ...
        if ($upPerm->getResultado()) {
            $_SESSION['adms_msg'] = "<div class='alert alert-success'>Alterado a situação do menu com sucesso!</div>";
            $this->Resultado = true;
        } else {
            $_SESSION['adms_msg'] = "<div class='alert alert-danger'>Erro: Não foi alterado a situação do menu!</div>";
            $this->Resultado = false;
