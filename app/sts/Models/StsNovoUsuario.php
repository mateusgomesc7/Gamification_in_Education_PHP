...
class StsNovoUsuario
{
    ...
    public function cadUser(array $Dados)
    {
        ...
        if ($this->Resultado) {
            ...
            $valSenha = new \App\sts\Models\helper\StsValSenha();
    ...
    private function inserir()
    {
        ...
        $this->Dados['sts_niveis_acesso_id'] = $this->InfoCadUser[0]['sts_niveis_acesso_id'];
        $this->Dados['sts_sits_usuario_id'] = $this->InfoCadUser[0]['sts_sits_usuario_id'];
        $this->Dados['created'] = date('Y-m-d H:i:s');
        $cadUser = new \App\sts\Models\helper\StsCreate();
        $cadUser->exeCreate('sts_usuarios', $this->Dados);
    ...
    private function infoCadUser()
    {
        ...
        $infoCadUser->fullRead("SELECT env_email_conf, sts_niveis_acesso_id, sts_sits_usuario_id FROM sts_cads_usuarios WHERE id =:id LIMIT :limit", "id=1&limit=1");
    ...
    private function dadosEmail()
    {
        ...
        $this->DadosEmail['cont_email'] .= "<a href='" . URL . "confirmar/confirmar_email?chave=" . $this->Dados['conf_email'] . "'>Clique aqui</a><br><br>";
        ...
        $this->DadosEmail['cont_text_email'] .= URL . "confirmar/confirmar_email?chave=" . $this->Dados['conf_email'];
